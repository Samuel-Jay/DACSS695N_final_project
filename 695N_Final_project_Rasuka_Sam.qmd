...
layout_coords <- layout_with_fr(collab_graph)
...
# Overlay squares for nodes that meet the condition
squares_nodes <- V(collab_graph)[str_detect(V(collab_graph)$genres, "hip hop") & !(str_detect(V(collab_graph)$genres, "pop"))]
points(
  x = layout_coords[squares_nodes, 1],
  y = layout_coords[squares_nodes, 2],
  pch = 22,
  bg = "blue",
  cex = 2
)
...
The nodes ranked by eigencentrality:

```{r}
head(centrality_followers %>% 
  arrange(desc(eigencentrality)))
```

The nodes ranked by follower counts:

```{r}
head(centrality_followers %>% 
  arrange(desc(follcts)))
```

The only name that is in both top 6 entries is Daddy Yankee. Everyone else is in one of the tables but not the other. This shows that Eigencentrality is not very strongly correlated to follower counts.
...
## Community detection and cross-community vs in-community collaboration:
...
total_nodes <- vcount(collab_graph)

# Get the sizes of each community
community_sizes <- sizes(communities)

# Calculate the proportion of nodes in each community
proportions <- community_sizes / total_nodes

# Print the proportions
print(proportions)
```

There are 4 communities found. Above are the percentage of artists in each community.

```{r}
# Calculate closeness centrality for each node
closeness <- closeness(collab_graph)

# Calculate the average closeness centrality within each community
avg_closeness_within <- tapply(closeness, membership(communities), mean)

# Calculate the average closeness centrality between different communities
# by iterating over each edge and averaging the closeness of nodes in different communities
avg_closeness_between <- mean(sapply(E(collab_graph), function(e) {
  from_community <- membership(communities)[as.integer(tail(e, 1))]
  to_community <- membership(communities)[as.integer(head(e, 1))]
  if (!is.na(from_community) && !is.na(to_community) && from_community != to_community) {
    (closeness[tail(e, 1)] + closeness[head(e, 1)]) / 2
  } else {
    0  # Ignore edges within the same community or with missing community membership
  }
}))

# Print the results
print("Average closeness within each community:")
print(avg_closeness_within)
print("Average closeness between different communities:")
print(avg_closeness_between)
```
...
We were also unable to do proper regression on node attributes as a function of centrality measures. We did some basic analysis.
