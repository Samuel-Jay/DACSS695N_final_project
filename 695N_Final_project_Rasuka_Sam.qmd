...
=======
```{r}
hiphop <- right_join(hiphop, edges, join_by(spotify_id == id_0))

pop <- right_join(pop, edges, join_by(spotify_id == id_0))
```

```{r}

hiphopel <- as.matrix(data.frame(hiphop[,1],hiphop[,7]))
hiphopnet <- graph_from_edgelist(hiphopel, directed = FALSE)

popel <- as.matrix(data.frame(pop[,1],pop[,7]))
popnet <- graph_from_edgelist(popel, directed = FALSE)

hiphopnet <- asNetwork(hiphopnet)
popnet <- asNetwork(popnet)
```
>>>>>>> 4fb82f16ef0a4aaf137a9fad8aead7afee994064
