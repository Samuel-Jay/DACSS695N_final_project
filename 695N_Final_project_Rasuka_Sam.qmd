...
hiphop1 <- hiphop %>% distinct(id_0, .keep_all = TRUE)
hiphop2 <- hiphop %>% distinct(id_1, .keep_all = TRUE)
hiphop <- anti_join(hiphop1, hiphop2, by = join_by(id_0))

pop1 <- pop %>% distinct(id_0, .keep_all = TRUE)
pop2 <- pop %>% distinct(id_1, .keep_all = TRUE)
pop <- anti_join(pop1, pop2, by = join_by(id_0))
...
hiphop_collab_graph <- graph_from_data_frame(hiphop)
pop_collab_graph <- graph_from_data_frame(pop)
mean(degree(hiphop_collab_graph))
mean(degree(pop_collab_graph))

hiphopnames <- hiphop %>% filter(hiphop$id_0 %in% V(hiphop_collab_graph)$name &
                                   hiphop$id_1 %in% V(hiphop_collab_graph)$name)
...
> > > > > > > 6fc56b727656699289a7306e2af2421fa658102c
